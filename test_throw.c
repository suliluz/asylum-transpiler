...
int main(int argc, char** argv) {
    ...
    tape[10] = (unsigned char)argc;
    time_t t = time(NULL);
    struct tm tm = *localtime(&t);
    tape[11] = (unsigned char)((tm.tm_year + 1900) % 100);
    tape[12] = (unsigned char)(tm.tm_mon + 1);
    tape[13] = (unsigned char)tm.tm_mday;
    tape[14] = (unsigned char)tm.tm_hour;
    tape[15] = (unsigned char)tm.tm_min;
    tape[16] = (unsigned char)tm.tm_sec;
    int offset = 17;
    ...
    (*ptr) += 1;
    ptr -= 1026; if (ptr < tape) ptr += TAPE_SIZE;
    while (*ptr) {
    (*ptr) -= 1;
    }
    (*ptr) += 1;
    ...
    while (*ptr) {
    (*ptr) -= 1;
    ptr -= 1025; if (ptr < tape) ptr += TAPE_SIZE;
    (*ptr) += 1;
    ptr += 1026; if (ptr >= tape + TAPE_SIZE) ptr -= TAPE_SIZE;
    (*ptr) += 1;
    ptr -= 1; if (ptr < tape) ptr += TAPE_SIZE;
